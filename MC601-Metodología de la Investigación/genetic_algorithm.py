...
def f(X):
    # X: [UNION OF EVERY RESTRICTED NODE WITH SOME OThER NODES (BIT REPRESENTATION)] + [ADJACENCY MATRIX]
    # PASS X AS A (MATRIX -> VECTOR) ADJACENCY MATRIX?
    Elements = []
    adjacency = make_adjacency(X, Elements)

    # VERIFY IF ALL RESTRICTED NODES ARE IN THE SAME CONNECTED COMPONENT
    # DELETE ALL OTHER COMPONENTS
    # VERIFY IF THE MATRIX IS CONSISTENT AND ERASE ALL OTHER NON-CONSISTENT CONNECTIONS
